  ...
  const io = new Server(httpServer, {
    cors: {
      origin: 'http://192.168.10.29:3005',
...
if (!client.info || !client.info.wid || !client.info.wid.user) {
  ...
  //cors
  app.use(cors({
    origin: 'http://192.168.10.29:3005', // ✅ NOT '*'
  ...
  if (result.rows.length === 0) {
    console.log('⚠️ No license found, inserting temporary license');
    await pool.query(
  'INSERT INTO license_keys (whatsapp_number, is_active, hashed_key) VALUES ($1, $2, $3)',
  [scannedNumber, true, 'TEMPORARY_KEY']
);
