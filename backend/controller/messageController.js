...
exports.sendTextOrImage = async (req, res) => {
  console.log("📨 sendTextOrImage called");

  const { numbers, message, category, userId, group } = req.body;
  ...
  try {
    ...
    if (group) {
      const groupId = group.endsWith('@g.us') ? group : `${group}@g.us`;
      ...
      await pool.query(
        ...
        [message, [groupId], null, userId || null, image]
      );

      return res.json({ success: true }); // ✅ Exit after sending to group
    }
...
exports.sendVoice = async (req, res) => {
  ...
  try {
    ...
    for (const number of contactsNumbers) {
const formatted = number.includes('@g.us') ? number : number.replace(/\D/g, '') + '@c.us';
