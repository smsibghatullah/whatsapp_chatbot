  ...
  try {
    const result = await pool.query(`
      SELECT contacts.id, contacts.name, contacts.number,
             COALESCE(json_agg(json_build_object('id', categories.id, 'name', categories.name))
  ...
  try {
    ...
    try {
      ...
      const existing = await client.query(
        'SELECT id FROM contacts WHERE number = $1',
        [number]
      );
      ...
      if (Array.isArray(categories) && categories.length > 0) {
        const parsedCategories = categories.map(Number);
        const insertCategoriesQuery = `
          INSERT INTO contact_categories (contact_id, category_id)
          VALUES ${parsedCategories.map((_, i) => `($1, $${i + 2})`).join(', ')}
        `;
        await client.query(insertCategoriesQuery, [contactId, ...parsedCategories]);
      }
    ...
    console.error('Error adding contact:', err.message, err.stack);
    res.status(500).json({ error: 'Internal server error' });
  }
};

// Update contact (optional fix similar logic can be added here)
...
const deleteContact = async (req, res) => {
  ...
  try {
    ...
    console.error('Error deleting contact:', err.message, err.stack);
    res.status(500).json({ error: 'Internal server error' });
  }
};
